...
class SageMakerJobManager:
    ...
    def stop_job_by_type(self, job_type, job_name):
        """Stop a job based on its type"""
        if job_type == 'Training':
            return self.stop_training_job(job_name)
        elif job_type == 'Processing':
            return self.stop_processing_job(job_name)
        elif job_type == 'Transform':
            return self.stop_transform_job(job_name)
        elif job_type == 'HyperparameterTuning':
            return self.stop_hyperparameter_tuning_job(job_name)
        elif job_type == 'AutoML':
            return self.stop_automl_job(job_name)
        elif job_type == 'Compilation':
            return self.stop_compilation_job(job_name)
        elif job_type == 'EdgePackaging':
            return self.stop_edge_packaging_job(job_name)
        elif job_type == 'Labeling':
            return self.stop_labeling_job(job_name)
    ...
    def list_and_stop_interactive(self):
        """List jobs as found and allow immediate stopping"""
        print("\n" + "="*80)
        print("SCANNING FOR SAGEMAKER JOBS")
        print("="*80)
        
        stopped_count = 0
        job_count = 0
        
        # Check most common job types first (Training and Processing)
        job_types_to_check = [
            ('Training', self.list_training_jobs),
            ('Processing', self.list_processing_jobs),
            ('Transform', self.list_transform_jobs),
            ('HyperparameterTuning', self.list_hyperparameter_tuning_jobs),
            ('AutoML', self.list_automl_jobs),
            ('Compilation', self.list_compilation_jobs),
            ('EdgePackaging', self.list_edge_packaging_jobs),
            ('Labeling', self.list_labeling_jobs),
        ]
        
        for job_type_name, list_func in job_types_to_check:
            print(f"Checking {job_type_name} jobs...", end='', flush=True)
            jobs = list_func()
            ...
            if jobs:
                ...
                for job in jobs:
                    ...
                    print(f"  Instance: {job['instance_type']}")
                    print(f"  Name:     {job['name']}")
                    print(f"  Created:  {creation_time}")
                    print(f"{'='*80}")
                    
                    response = input(f"\n⚠️  Stop this job? (y/n/q to quit): ").strip().lower()
                    if response == 'q':
                        print("\nQuitting...")
                        print(f"\n✓ Stopped {stopped_count}/{job_count} job(s)")
                        return
                    elif response in ['y', 'yes']:
                        if self.stop_job_by_type(job['type'], job['name']):
                            stopped_count += 1
                    else:
                        print("Skipping...")
            else:
...
def main():
    ...
    parser.add_argument(
        '--action',
        type=str,
        choices=['interactive', 'list', 'stop-all'],
        default='interactive',
        help='Action to perform: interactive (default, stop jobs as found), list (scan all first), or stop-all (stop without asking)'
    )
    parser.add_argument(
        '--no-confirm',
        action='store_true',
    ...
    if args.action == 'interactive':
        manager.list_and_stop_interactive()
